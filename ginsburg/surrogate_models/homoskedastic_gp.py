...
class HomoskedasticGP(gpytorch.models.ApproximateGP):
    ...
    def __init__(self, D, inducing_points, name_prefix="heteroskedastic_gp"): # D is the dimensionality of the problem
        ...
        variational_strategy = gpytorch.variational.VariationalStrategy(
                self, inducing_points, variational_distribution, learn_inducing_locations=False) # False for the full GP

        # Standard initializtation
        super().__init__(variational_strategy)
        self.mean_module = gpytorch.means.ConstantMean()         
        self.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel(ard_num_dims=D)) # ScaleKernel to add kernel variance
    ...
    def model(self, x, y):
        ...
        # Use a plate here to mark conditional independencies.
        # Our samples are of shape (nobs, D), therefore the dim=-2
        # in the plate:
...
# training routine:
def train_homoskedastic(model, train_x, train_y, num_particles, num_iter):
    model.train()
    
    # optimizer = torch.optim.Adam
    optimizer = pyro.optim.Adam({"lr": 0.01})
    # scheduler = pyro.optim.ExponentialLR({'optimizer': optimizer, 'optim_args': {"lr": 0.1}, 'gamma': 0.3})
    ...
    for i in pbar:
        ...
        pbar.set_postfix(ELBO=loss) #, lengthscale=model.covar_module.base_kernel.lengthscale) # only makes sense for single lengthscale (no ARD)
