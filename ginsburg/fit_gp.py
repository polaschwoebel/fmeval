...
from surrogate_models.homeoskedastic_gp import HomeoskedasticGP, train_homeoskedastic
import torch
import gpytorch
...
def fit_gp(args):
    ...
    model = HomeoskedasticGP(inducing_points=inducing_points, D=D, name_prefix="homeoskedastic_gp")
    
    num_iter = 1000
    num_particles = 512 # 256
    
    # Train the GP:
    model = train_homeoskedastic(model,  X_train, y_train, num_particles, num_iter)
    ...
    y_pred = sigmoid(output_samples)
    
    test_acc = (np.array((y_pred.mean(0) > 0.5)).astype(bool)  == y_test).mean()
    breakpoint()
